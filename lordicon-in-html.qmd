...
Click on any icon, I'm choosing *Confetti* (in the *Popular* set). You'll set the icon information pop up on the right. Hit the `</> HTML` button on the bottom of that info listing and a popover will appear. What you need is the **eight-letter code**, and, you need to transpose that to lowercase. Now, you finally have the code you need to successfully use the icon in the shortcode.

Let's celebrate by using that *Confetti* icon (`"xxdqfhbi"`).

{{< li xxdqfhbi >}} 

Each style of will have an entirely different eight-letter code.
...
The `speed` argument controls how quickly the icon's animation plays. Let's use a different style for the *Confetti* icon here (code obtained from the website using the methodology in the first section).
